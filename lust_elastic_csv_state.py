# -*- coding: utf-8 -*-
"""
Created on Tue Aug  6 15:58:21 2024
...
logging.basicConfig(filename='lust_export.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
index_name = "eos_axon_lust_index"
...
output_dir = "output_lust_csvs"
zip_dir = 'lust_zip_files'
                  ...
                  "sort": [
                    {
                      "_script": {
                        ...
                        "script": {
                          "lang": "painless",
                          "source": "doc['id.keyword'].value != null ? Integer.parseInt(doc['id.keyword'].value) : 0"
                        },
                        "order": "asc"
                      }
                    }
...
zip_file_name = f"axon_LUST_details_{datetime.now().strftime('%Y%m%d%H%M%S')}.zip"
...
logging.info(f"Lust data exported and zipped in file: {zip_file_name}")

upload_to_sftp('lust_zip_files','EOX_axon_lustdata',zip_file_name)
