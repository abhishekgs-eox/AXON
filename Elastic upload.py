...
def build_doc(row, doc_id):
    ...
    doc["year_installed_keyword"] = cleaned_year_inst  # Optional: for future mapped keyword use
...
# This is the place where we REMOVE "_type": rec["_type"] as recommended.
def elastic_bulk_actions(json_batch):
    for rec in json_batch:
        yield {
            "_index": rec["_index"],
            # "_type": rec["_type"],  # removed for ES 7+
            "_id": rec["_id"],
            "_source": rec["_source"]
        }

def excel_to_json_and_elastic(file_path, export_folder, start_id, es):
    df = pd.read_excel(file_path, dtype=str).fillna('')
    total_docs = len(df)
    file_name = os.path.basename(file_path)
    logging.info(f"Processing file '{file_name}' with {total_docs} records starting id={start_id}")
    batch_number = 1
    for batch_start in tqdm(range(0, total_docs, BATCH_SIZE), desc=f"{file_name} (Export+Elastic)"):
        ...
        try:
            helpers.bulk(es, elastic_bulk_actions(json_batch))
            logging.info(f"{file_name}: Uploaded batch {batch_number} (doc_ids {start_id + batch_start} - {start_id + batch_start + len(json_batch) - 1})")
        except Exception as e:
            logging.error(f"{file_name}: Error uploading batch {batch_number}: {e}")
        batch_number += 1

if __name__ == "__main__":
    print("----- Excel to JSON + Elastic Upload Begin -----")
    start_id = 1
    es = Elasticsearch(
    ...
    for file in files:
        ...
        excel_to_json_and_elastic(file_path, export_folder, start_id, es)
        with pd.ExcelFile(file_path) as xls:
            n_rows = pd.read_excel(xls, dtype=str).shape[0]
        start_id += n_rows
    print("----- All Excel data exported and ingested to Elastic. -----")
    print(f"See log file (date corrections, warnings, errors): {log_file}")